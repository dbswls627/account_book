...
public class adapter2 extends RecyclerView.Adapter<adapter2.CumstomViewHolder>{
    ...
    public void onBindViewHolder(@NonNull adapter2.CumstomViewHolder holder, int position) {
        ...
        if(!arrayList.get(position).getMsgBody().equals("") && arrayList.get(position).getMsgAmount()!=-1) { //정규식에 예외처리가 되지 않으면..
            holder.dt.setText(arrayList.get(position).getMsgDate().substring(1,15));
        }

        ArrayList<item> Array = new ArrayList<>();
        for (item item : arrayList) {  //arrayList 의 날짜 값이랑 같은 값만 adapter로 넘겨주기 위함
            if(item.getMsgDate().substring(1,15).equals(arrayList.get(position).getMsgDate().substring(1,15))){
                Array.add(item);
            }
